...
import { SeriesAnalyzer } from '../../utilities/SeriesAnalyzer'
...
async function calculateAllSeriesWithTweets(tweets) {
    ...
    const data = {
        dailyScatterSeries: [
            { name: 'Male', data: SeriesAnalyzer.createDailyScatterSeriesFor(maleTweets) },
            { name: 'Female', data: SeriesAnalyzer.createDailyScatterSeriesFor(femaleTweets) }
        ],
        dailySentimentSeries: [
            { name: 'Male', data: SeriesAnalyzer.createAverageHourlySeriesFor(maleTweets).hourly },
            { name: 'Male (Adjusted for follower count)', data: SeriesAnalyzer.createAverageHourlySeriesFor(maleTweets).hourlyWeighted },
            { name: 'Female', data: SeriesAnalyzer.createAverageHourlySeriesFor(femaleTweets).hourly },
            { name: 'Female (Adjusted for follower count)', data: SeriesAnalyzer.createAverageHourlySeriesFor(femaleTweets).hourlyWeighted }
        ],
        dailyBreakdownSeries: [
            { name: 'Male', data: SeriesAnalyzer.createHourlyTotalSeriesFor(maleTweets) },
            { name: 'Female', data: SeriesAnalyzer.createHourlyTotalSeriesFor(femaleTweets) }
        ],
        wordChartSeries: [
            SeriesAnalyzer.aggregateWordsUsedIn(maleTweets),
            SeriesAnalyzer.aggregateWordsUsedIn(femaleTweets)
        ]
    }
    return await data
}
